...
const MapFrame = withScriptjs(
  withGoogleMap(({ children }) => (
    <GoogleMap
      defaultZoom={8}
      defaultCenter={{ lat: 47.548, lng: -121.9836 }}
      defaultOptions={{
...
const HRAMultiPolygon = ({ hraId, active, toggleActive, color }) => {
  const toggle = useCallback(() => toggleActive(hraId), [hraId]);
  return polygonsCoordsByHRAId[hraId].map((polygonCoordinates, i) => (
    <Polygon
      key={i}
      paths={polygonCoordinates.map((path) =>
        path.map(([lng, lat]) => ({ lng, lat })),
      )}
      options={{
