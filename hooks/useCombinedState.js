    ...
    .mapValues(
      (
        ...
        const interpolatedData = prevoiusAndNextData.reduce(
          (data, [previousDatum, nextDatum]) => {
            ...
            while (date < nextDatum.date) {
              ...
              const millisToPreviousDate = differenceInMilliseconds(
                previousDatum.date,
                date,
              );
              const millisToNextDate = differenceInMilliseconds(
                date,
                nextDatum.date,
              );
              const previousWeight =
                millisToNextDate / (millisToPreviousDate + millisToNextDate);
              const nextWeight =
                millisToPreviousDate /
                (millisToPreviousDate + millisToNextDate);
              ...
              interpolatedFields.forEach((fieldName) => {
                ...
                datum[fieldName] = Math.round(
                  previousDatum[fieldName] * previousWeight +
                    nextDatum[fieldName] * nextWeight,
                );
              });
        ...
        const smoothedInterpolatedData = interpolatedData.map((datum, i) => {
          const smoothingData = interpolatedData.slice(
            i + 1 - smoothingWindowInDays,
            i + 1,
          );
          const smoothedFields =
            smoothingData.length === smoothingWindowInDays
              ? {
                  ...
                  μhospitalizations: mean(
                    smoothingData,
                    (d) => d.hospitalizations,
                  ),
...
export default function useCombinedState(covidData) {
  ...
  const setAllInactive = useCallback(() =>
    dispatch({ type: 'SET_ALL_INACTIVE' }),
  );
  const toggleActive = useCallback(
    (hraId) => dispatch({ type: 'TOGGLE_ACTIVE', hraId }),
    [dispatch],
  );
  const setHoveredHraId = useCallback(
    (hraId) => dispatch({ type: 'SET_HOVER', hraId }),
    [dispatch],
  );
